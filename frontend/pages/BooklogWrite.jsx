...
function BooklogWrite() {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedBook.itemId != null)
      formData.append("bookId", selectedBook.itemId);

    try {
      const res = isEdit
        ? await api.put(`/api/posts/${editingLog.id}`, formData, {
  ...
  const handleSaveBook = async (book) => {
    try {
      ...
      */
      ...
      console.error("Failed to save selected book:", error);
      /*
  ...
  const handleBookSelect = async (book) => {
    try {
      const saved = await handleSaveBook(book);
      setSelectedBook({
        title: saved.title,
        author: saved.author,
        isbn13: book.isbn13,
      ...
      console.error("Failed to save selected book:", err);
