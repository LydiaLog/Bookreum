...
function BooklogList() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await api.get("/api/posts"); // [{id, title, content, date, bookTitle, bookAuthor, coverUrl, nickname}]
        setLogs(data.content); // Page 객체에서 content 추출
      } catch (err) {
  ...
  const { pageData, totalPages } = useMemo(() => {
    ...
    let list = logs.filter((l) => {
      const hay =
        `${l.title} ${l.bookTitle} ${l.bookAuthor} ${l.nickname}`.toLowerCase();
            ...
            <div className="bookloglist">
              {pageData.map((data) => (
                <BooklogCard
                  key={data.id}
                  booklog={data}
                  onClick={() => navigate(`/booklog/${data.id}`)}
                />
              ))}
