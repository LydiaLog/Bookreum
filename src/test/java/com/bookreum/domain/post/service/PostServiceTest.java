...
class PostServiceTest {
    ...
    void createPost_throwsExceptionForMissingTitle() {
        ...
        assertThrows(DataIntegrityViolationException.class, () -> 
                postService.createPost(null, content, image, author, book));
    }

    @Test
    void updatePost_updatesSuccessfully() {
        Post post = Post.builder().title("기존 제목").content("기존 내용").book(book).user(author).build();
        em.persist(post);

        String newTitle = "업데이트된 제목";
        String newContent = "업데이트된 내용";

        postService.updatePost(post.getId(), newTitle, newContent, null, null);
